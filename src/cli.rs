...
struct ConnectionArgs {
    #[arg(long, global = true, env = "WIZ_API_ENDPOINT")]
    ...
    #[arg(long, global = true, env = "WIZ_AUDIENCE", default_value = "wiz-api")]
    audience: String,
    #[arg(long, global = true, env = "WIZ_CLIENT_ID")]
    client_id: Option<String>,
    #[arg(long, global = true, env = "WAND_TIMEOUT", default_value_t = 30,
        value_parser = clap::value_parser!(u64).range(1..=300))]
    timeout: u64,
    #[arg(long, global = true, env = "WAND_RETRIES", default_value_t = 2,
...
async fn paginated(
    ...
    loop {
        ...
        nodes.extend(page_nodes.iter().take(remaining).cloned());
        requests += 1;
        let info = page_value
            .get("pageInfo")
    ...
    Ok(envelope(
        ...
        json!({
            "count":nodes.len(),"truncated":truncated,"next_cursor":cursor,"requests":requests,"filter":filter
        }),
    ))
}
...
fn agent_schema() -> Value {
    envelope(
        json!({
        "name":"wand","version":env!("CARGO_PKG_VERSION"),"read_only":true,
        ...
        "schemas":{
            ...
            "commands":[
                ...
                {"path":"issues list","safety":"read","example":"wand issues list --severity CRITICAL,HIGH --status OPEN --limit 200","arguments":{"limit":"integer >= 1","page-size":"integer 1..500","max-pages":"integer 1..1000","cursor":"string?","severity":"string[]","status":"string[]","project":"string[]","type":"string[]","filter":"JSON object"}},
                {"path":"issues get <id>","safety":"read","arguments":{"id":"required string"}},
                {"path":"vulnerabilities list","safety":"read","arguments":{"limit":"integer >= 1","page-size":"integer 1..500","max-pages":"integer >= 1","cursor":"string?","severity":"string[]","status":"string[]","project":"string[]","has-exploit":"boolean?","updated-after":"timestamp?","filter":"JSON object"}},
